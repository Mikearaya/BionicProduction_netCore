...
export class DataViewComponent implements OnInit {
  ...
  initilizeCommandColumn(): void {
    if (this.showUpdate) {
      this.commands.push({
        buttonOption: { iconCss: 'e-icons e-edit', cssClass: 'e-flat', click: this.editAction.bind(this) }
      });
    }

    if (this.showDelete) {
      this.commands.push({ buttonOption: { iconCss: 'e-icons e-delete', cssClass: 'e-flat', click: this.deleteAction.bind(this) } });
    }
  }


  deleteAction(event: Event) {
    const rowObj: IRow<Column> = this.grid.getRowObjectFromUID(closest(<Element>event.target, '.e-row').getAttribute('data-uid'));
  ...
  actionEndHandler(args: ActionEventArgs) {
    ...
    this.grid.getColumns().filter(c => c.visible && c.field !== undefined).forEach(s => this.query.selectedColumns.push(s.field));
    ...
    if (this.query.pageSize !== this.grid.pageSettings.pageSize ||
    ...
    switch (args.item.id) {
      case 'dataview_add':
        if (this.addRoute.trim().length === 0) {
          ...
          this.router.navigate([this.addRoute], { relativeTo: this.activatedRoute });
        }
        break;
